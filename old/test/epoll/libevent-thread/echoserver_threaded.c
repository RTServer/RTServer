 ...
 *
 * To compile: gcc -o echoserver_threaded echoserver_threaded.c workqueue.c -I /opt/local/include -L/opt/local/lib -levent -lpthread
...
//保存客户端[start]
typedef struct client_fds {
	int fd;
	void *user_data;
}client_fds_t;
client_fds_t clients[CLINET_MAX_NUM];
int client_add(int fd, client_t *client) {
	int i;
	for (i = 0; i < CLINET_MAX_NUM; i++) {
		if (clients[i].fd == 0) {
			clients[i].fd = fd;
			clients[i].user_data = client;
			return i;
		}
	}
	return -1;
...
void buffered_on_read(struct bufferevent *bev, void *arg) {
	...
	//发送给另外一个客户端
	//bufferevent_setfd(client2->buf_ev, client2->fd);
	

	//send(client2->fd, data, strlen(data), 0);
