...
void xmlnode_init() {
    ...
    for(i = 0; i < MAX_XMLNODE_NUM; i++) {
        ...
        wmemset(_xmlnode[i].name, 0, MAX_XMLNAME_LENG + 1);
        wmemset(_xmlnode[i].text, 0, MAX_XMLTEXT_LENG + 1);
    ...
    for(curNode = aNode; curNode; curNode = curNode->next) {
        switch(curNode->type) {
            case XML_ELEMENT_NODE:
                ...
                mbstowcs(_xmlnode[_n].name, (char *)curNode->name, MAX_XMLNAME_LENG);
                ...
                if(attrPtr != NULL) {
                    ...
                    if(_xmlattr != NULL) {
                        ...
                        while(attrPtr != NULL) {
                            ...
                            mbstowcs(_xmlattr[i].key, (char *)attrPtr->name, MAX_XMLATTR_KEY_LENG);
                            wmemset(_xmlattr[i].value, 0, MAX_XMLATTR_VALUE_LENG + 1);
                            mbstowcs(_xmlattr[i].value, (char *)szAttr, MAX_XMLATTR_VALUE_LENG);
            ...
            case XML_TEXT_NODE:
                ...
                mbstowcs(_xmlnode[_n].text, (char *)szKey, MAX_XMLTEXT_LENG);
                xmlFree(szKey);
                break;
            default:
                break;
