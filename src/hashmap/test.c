...
static int iter_elem(void* elem, void *arg) {
  ...
  printf("key=%s; value=%s; test=%s\n", el->key, el->value, el->test);
  return 0;
}
...
int main(int argc, char* argv[])
{
  ...
  for (i=0; i<100; i++) {
    ...
    for (j=0; j<10; j++) {
      ...
      el->test = (char *)malloc(30);
      sprintf(el->test, "%d", j+10000);
  ...
  /* 删除指定元素: key="10" */
  ret = hashmap_remove(map, "10", (void **)&dat);
  assert(ret==HMAP_S_OK);
  ...
  /* 删除指定元素: key="11" */
  ret = hashmap_remove(map, "11", (void **)&dat);
  assert(ret==HMAP_S_OK);
  printf("hashmap_remove: name=%s. size=%d\n", dat->name, hashmap_size(map));
  hashmap_iterate(dat->map, iter_elem, 0);
  free_data(dat, 0);
  ...
  /* 查询元素: key="99" */
  ret = hashmap_get(map, "99", (void **)&dat);
  assert(ret==HMAP_S_OK);
